...
A100 box. Deps: torch transformers peft trl datasets accelerate.
  ...
  # one arm at a time
  python -m rampart.train.finetune --arm runs/sft_arm_naive.jsonl    --out adapters/naive
  python -m rampart.train.finetune --arm runs/sft_arm_hardened.jsonl --out adapters/hardened
...
def main() -> None:
    p = argparse.ArgumentParser(prog="rampart.train.finetune")
