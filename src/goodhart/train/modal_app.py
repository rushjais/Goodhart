...
Each arm becomes a LoRA adapter in the 'goodhart-adapters' Modal volume. After training, eval
...
image = (
    ...
    .add_local_python_source(
        "goodhart"
    )  # makes goodhart.train.finetune importable in the container
)
app = modal.App("goodhart-finetune", image=image)
adapters = modal.Volume.from_name("goodhart-adapters", create_if_missing=True)
...
def train(arm_name: str, rows: list[dict], epochs: int = 2) -> str:
    ...
    from goodhart.train.finetune import train_arm
...
def main(
    ...
    for name, path in (("naive", arm_naive), ("hardened", arm_hardened)):
        ...
        print(f"trained arm_{name} -> {out} (in the goodhart-adapters volume)")
