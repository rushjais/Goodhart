"""Map a HUD environment into a Goodhart EnvAdapter — run any dataset-style HUD env through the
...
def hud_adapter(
    ...
    *,
    ...
    """A HUD env as a Goodhart adapter: its (thresholded) reward is the verifier under test.

    `oracle` is the env's ground truth if it has one, else `derive_oracle(...)`. `hardened` is left
    None — Goodhart's harden loop produces it; until then the leaderboard compares naive vs naive.
    """
    return EnvAdapter(
        name=name,
