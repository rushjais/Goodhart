"""HUD trace-logging for the verifier leaderboard — make a Goodhart eval observable on hud.ai.
...
async def _traced(rollouts, verifiers, *, model, local_dir):
    ...
    await job_enter(job_id, name="goodhart-bench", group=1)
    ...
    with set_trace_context(trace_id):
        for name, verdict in verifiers:
            ...
            @hud.instrument(name=f"goodhart.verifier.{name}")
