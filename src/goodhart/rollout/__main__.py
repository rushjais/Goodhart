"""CLI: produce the multi-model rollout dataset (the locked-seam JSONL).
  ...
  python -m goodhart.rollout --models opus,sonnet,haiku --count 8 --k 4 --out runs/rollouts.jsonl
  python -m goodhart.rollout --mock --models opus,haiku --count 2          # mock rewards
  python -m goodhart.rollout --rg --models opus,sonnet,haiku --count 30          # reasoning-gym
  python -m goodhart.rollout --rg --red --models opus,haiku --count 30 --k 4      # + real cheats
  python -m goodhart.rollout --hardest --count 25 --models "" --fw-pressure \
...
def main() -> None:
    p = argparse.ArgumentParser(prog="goodhart.rollout")
